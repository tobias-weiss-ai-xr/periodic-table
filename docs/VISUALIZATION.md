...
Each element gets its own small chamber (`assets/room.js` + one generated
HTML per element): a giant Bohr atom on a pedestal, the element's fact card
and summary floating as holograms, free flight + WebXR, and a **return door**
(also in the HUD's *← Periodic Table Room* link) leading back to the gallery.
...
| `lib/primitives.js` | `makeLabel`, `makeGlow`, shared geometry, `buildAtom`, `buildDoor` |
